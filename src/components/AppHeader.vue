...
<template>
  ...
  >
    <div class="mx-auto flex max-w-7xl items-center gap-4 px-4 py-4 sm:px-6 lg:px-8">
      <button
        class="group flex shrink-0 items-center gap-3 cursor-pointer"
        aria-label="返回首页"
      ...
      <nav class="hidden flex-1 justify-center lg:flex">
        ...
        >
          <button
            ...
            :key="item"
            class="relative rounded-full px-5 py-2.5 text-sm font-semibold transition-all duration-300 cursor-pointer"
            :class="
              activeNav === item
      ...
      <!-- Search Bar -->
      <div
        class="ml-auto hidden w-[300px] max-w-[34vw] items-center gap-2 rounded-full border border-brand-border bg-brand-card/60 px-4 py-2.5 text-brand-text-secondary shadow-inner backdrop-blur-xl md:flex transition-colors duration-300"
      >
        <Search class="h-4 w-4 text-[#ff6f98]" />
        <input
          class="w-full bg-transparent text-sm text-brand-text outline-none placeholder:text-brand-text-secondary/50"
    ...
    <div class="scrollbar-none flex gap-2 overflow-x-auto px-4 pb-4 lg:hidden">
      <button
        v-for="item in navItems"
        :key="item"
        class="whitespace-nowrap rounded-full border px-4 py-2 text-sm font-semibold transition cursor-pointer"
        :class="
          activeNav === item
            ? 'border-brand-primary bg-brand-primary/10 text-brand-primary shadow-[0_0_18px_rgba(255,51,102,0.15)]'
            : 'border-brand-border bg-brand-card/50 text-brand-text-secondary'
        "
        @click="emit('change-nav', item)"
      >
        {{ item }}
